...
from src.core.ai_engine_operator import get_response_operator, get_operator_engine
...
class ChatAPI:
    def __init__(self):
        self.window = None
        self.temp_files = []
        self.operator_mode = True  # Start in operator mode by default
    
    def set_window(self, window):
        self.window = window
    
    def toggle_operator_mode(self):
        """Toggle between operator and chat mode"""
        self.operator_mode = not self.operator_mode
        mode = "Operator" if self.operator_mode else "Chat"
        print(f"✓ Switched to {mode} mode")
        return {"mode": mode, "operator_mode": self.operator_mode}
    ...
    def send_message(self, message, files=None):
            ...
            for file_data in files:
                try:
                    ...
                    if ext in ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp', '.ico']:
                        ...
                        print(f"✓ Image: {file_name} -> {temp_file.name}")
                    else:
                        document_paths.append(temp_file.name)
                        print(f"✓ Document: {file_name} -> {temp_file.name}")
        ...
        if self.operator_mode:
            # Operator mode - use planner
            response = get_response_operator(message)
        else:
            # Chat mode - use regular AI with images and documents
...
def _build_window(api):
    ...
    if os.path.exists(glb_path):
        try:
            ...
            print(f"✓ Fox 3D model embedded in HTML ({len(glb_data)} chars)")
        except Exception as e:
            print(f"✗ Error embedding 3D model: {e}")
            import traceback
            traceback.print_exc()
    else:
        print(f"✗ Fox_draco.glb not found at: {glb_path}")
    ...
    temp_html.close()
    print(f"✓ Saved modified HTML to: {temp_html.name}")
...
def _set_window_icon():
    ...
    try:
        ...
        if not os.path.exists(fox_png_path):
            print("✗ fox.png not found for icon")
            return
        ...
        if not hwnd:
            print("✗ Could not find window by title after 10 attempts")
            return
        ...
        print(f"✓ Window style updated to enable taskbar minimize/restore (frameless)")
        ...
        if hicon_small and hicon_big:
            ...
            print(f"✗ Could not load icons (small: {hicon_small}, big: {hicon_big})")
...
def create_chat_window():
    ...
    webview.start(on_ready, debug=True, private_mode=False)
    return window
